...
def find_L22_ip_in_output(output):
...
def find_hostname_in_output(output):
    for line in output.split("\n"):
...
def find_local_as_in_output(output):
    for line in output.split("\n"):
...
def find_site_code_in_hostname(hostname):
    k = re.match(SITE_CODE_RE, hostname)
    if k:
        site_code = k.group(1)
        return site_code

def play_with_device_session(net_connect, commands):
    output = '0' + '\n'
...
def find_peer_ip_and_as_in_output(net_connect,local_as):
...
def find_peer_ip_and_as_in_output2(net_connect, local_as):
...
def find_the_advertised_subnets(net_connect, cmd):
    output = play_with_device_session(net_connect, cmd)
    ADV_SUB_LIST = []
...
def append_data(file_path, ip, site_code, router_name, L22ip, peer_ip, peer_as, 
...
def create_session(ip):
    session = ConnectHandler(device_type='cisco_ios',ip=ip, username='amit',password='cisco')
    return session
...
def main():
    ...
    for ip in ip_addresses:
        p = Pool(10)
        # devices = p.map(create_device_details, ip)
        try:
            net_connect = p.map(create_session, ip)
        ...
        op1 = p.map(play_with_device_session, net_connect, show_commands)
        net_connect.disconnect()
        # print(op1)
        L22ip = p.map(find_L22_ip_in_output, op1)
        hostname1 = hostprompt.strip('#')
        site_code = p.map(find_site_code_in_hostname, hostname1)
        local_as = p.map(find_local_as_in_output, op1)
        peer_ip_as_list = p.map(find_peer_ip_and_as_in_output, net_connect, local_as)
        if peer_ip_as_list == None:
            peer_ip_as_list = p.map(find_peer_ip_and_as_in_output2, net_connect, local_as)
        peer_ip = peer_ip_as_list[0]
        peer_as = peer_ip_as_list[1]
        cmd1 = 'sh ip bgp neigh '+ str(peer_ip) + ' advertised-routes | in 32768'
        subnets = p.map(find_the_advertised_subnets, net_connect, cmd1)
        p.close()
        p.join()
        append_data('brbgpdata.csv', ip,site_code.upper(),hostname1,L22ip,peer_ip,peer_as,
        subnets[0], subnets[1], subnets[2], subnets[3], subnets[4], 
        subnets[5], subnets[6], subnets[7], subnets[8], subnets[9], subnets[10])
